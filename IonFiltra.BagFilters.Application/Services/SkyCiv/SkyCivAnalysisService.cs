...
namespace IonFiltra.BagFilters.Application.Services.SkyCiv
{
    public class SkyCivAnalysisService : ISkyCivAnalysisService
    {
        ...
        public async Task<AnalysisResponseDto> RunBOMAsync(JObject s3dModel, CancellationToken ct)
        {
            if (s3dModel == null) throw new ArgumentNullException(nameof(s3dModel));
            ...
            // 2. Build payload
            var payload = BuildBOMPayload(s3dModel, _opts.Username, _opts.Key);
            ...
            {
                // Note: PostForAnalysisAsync expected signature: Task<JObject> PostForAnalysisAsync(object payload, CancellationToken ct = default)
                result = await _client.PostBOMAsync(payload, ct);
            }
            catch (OperationCanceledException oce) when (ct.IsCancellationRequested)
            {
                _logger.LogWarning(oce, "SkyCiv Bill of Material cancelled by caller.");
                session.Status = "Cancelled";
                session.ErrorMessage = "Cancelled";
        ...
        private object BuildBOMPayload(JObject s3dModel, string username, string key)
        {
            // You can build a strongly typed DTO instead of object; keeping it dynamic/object for brevity.
            var plateIds = s3dModel["plates"] != null
                ? s3dModel["plates"].ToObject<Dictionary<string, object>>().Keys.Select(k => int.Parse(k)).ToArray()
            ...
            var customScript = $@"
                var s3d_model = getJSONOutput();
                s3d_model = S3D.API.S3D2API(s3d_model);
                let bom_obj = SKYCIV_APPS.s3d_bom.calculate(s3d_model, {sectionkey});
                API_RETURN_DATA = JSON.parse(JSON.stringify(bom_obj));
                ";

            var scriptObj = new { script = customScript };
