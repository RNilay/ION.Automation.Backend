...
namespace IonFiltra.BagFilters.Infrastructure.Repositories.BagfilterDatabase.WithCanopy
{
    public class IFI_Bagfilter_Database_With_CanopyRepository : IIFI_Bagfilter_Database_With_CanopyRepository
    {
        ...
        decimal? numberOfColumns)
        {
            return await _transactionHelper.ExecuteAsync(async dbContext =>
            {
                _logger.LogInformation("Fetching IFI_Bagfilter_With_Canopy by match criteria (3-param).");

                // ── Step 1: exact volume + all filters ─────────────────────────────
                var exactResult = await BuildQueryWithCanopy(dbContext, processVolume, hopperType, numberOfColumns)
                    .OrderByDescending(x => x.CreatedAt)
                    .FirstOrDefaultAsync();
                ...
                var allRows = await dbContext.IFI_Bagfilter_Database_With_Canopys
                    .AsNoTracking()
                ...
                if (nextGreaterVolumeStr == null)
                {
                    _logger.LogInformation(
                        "No next-greater volume found above '{Volume}' in With_Canopy table.", processVolume);
                    return null;
                }
                ...
                var result = await BuildQueryWithCanopy(dbContext, nextGreaterVolumeStr, hopperType, numberOfColumns)
                    .OrderByDescending(x => x.CreatedAt)
                    .FirstOrDefaultAsync();
                if (result != null)
                {
                ...
                result = await BuildQueryWithCanopy(dbContext, nextGreaterVolumeStr, hopperType, numberOfColumns: null)
                    .OrderByDescending(x => x.CreatedAt)
                    .FirstOrDefaultAsync();
                if (result != null)
                {
                ...
                result = await BuildQueryWithCanopy(dbContext, nextGreaterVolumeStr, hopperType: null, numberOfColumns)
                    .OrderByDescending(x => x.CreatedAt)
                    .FirstOrDefaultAsync();
                if (result != null)
                {
                ...
                result = await BuildQueryWithCanopy(dbContext, nextGreaterVolumeStr, hopperType: null, numberOfColumns: null)
                    .OrderByDescending(x => x.CreatedAt)
                    .FirstOrDefaultAsync();
        ...
        private static IQueryable<IFI_Bagfilter_Database_With_Canopy> BuildQueryWithCanopy(
        ...
        public async Task<bool> ExistsByProcessVolumeAsync(string processVolume)
        {
            return await _transactionHelper.ExecuteAsync(async dbContext =>
            {
                _logger.LogInformation("Checking existence of Process_Volume_m3hr={ProcessVolume} in With_Canopy table.", processVolume);
        ...
        /// </summary>
        public async Task<IFI_Bagfilter_Database_With_Canopy?> GetByNextGreaterVolumeAsync(decimal processVolume)
        {
            return await _transactionHelper.ExecuteAsync(async dbContext =>
            {
                _logger.LogInformation(
                    "Fetching next-greater Process_Volume row above {ProcessVolume} in With_Canopy table.",
                    processVolume);
                ...
                var candidates = await dbContext.IFI_Bagfilter_Database_With_Canopys
                    .AsNoTracking()
                    .Where(x => x.Process_Volume_m3hr != null)
                    .ToListAsync();
