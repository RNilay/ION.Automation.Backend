...
namespace IonFiltra.BagFilters.Infrastructure.Repositories.Bagfilters.Sections.Roof_Door
{
    public class RoofDoorRepository : IRoofDoorRepository
    {
        ...
        public async Task<Dictionary<int, RoofDoor>> GetByMasterIdsAsync(
    ...
    CancellationToken ct = default)
        {
            ...
            if (masterIdList.Count == 0)
                return new Dictionary<int, RoofDoor>();

            return await _transactionHelper.ExecuteAsync(async dbContext =>
            {
                var items = await dbContext.RoofDoors
                    .Where(r => masterIdList.Contains(r.BagfilterMasterId))
                    .ToListAsync(ct);

                // assuming 1:1 (one RoofDoor per BagfilterMaster)
                return items.ToDictionary(r => r.BagfilterMasterId, r => r);
            });
        }
            ...
            CancellationToken ct = default)
        {
            ...
            await _transactionHelper.ExecuteAsync(async dbContext =>
            {
                ...
                var existing = await dbContext.RoofDoors
                    .Where(r => masterIds.Contains(r.BagfilterMasterId))
                    .ToListAsync(ct);

                var existingByMasterId = existing.ToDictionary(r => r.BagfilterMasterId, r => r);
                ...
                foreach (var incoming in list)
                {
                    ...
                    else
                    {
                        ...
                        await dbContext.RoofDoors.AddAsync(incoming, ct);
                    }
                }
